...
public class TestDemo {
	...
	public void testSong() {
		...
		System.out.println("song4==song3："+(song3.equals(song4)));
	}
	...
	public void testPlayList() {
		...
		//通过名称查询歌曲信息
		song = mainPlayList.searchSongByName("两只老虎");
		if(song != null) {
	...
	public void testPlayListCollection() {
		...
		plc.displayListName();
		//根据播放列表的名字查询播放列表的信息，并显示所有歌曲
		PlayList playList = plc.searchPlayList("最喜欢的歌曲");
	...
	public void test() {
		...
		while(true) {
			td.mainMenu();
			System.out.println("请输入对于数字进行操作：");
			input=sc.nextInt();
			if(input==0) {
				break;
			}
			...
			case 1:
				...
				while(true) {
					...
					case 2:
						...
						//根据名称判断播放列表是否存在
						favouritePlayList = plc.searchPlayList(sName);
						if(favouritePlayList==null) {
							...
							for(int i=1; i<=count1;i++) {
								...
								if(song==null) {
									...
									//如果存在，则直接添加到现在的播放列表
									System.out.println("该歌曲已存在于主播放列表，已同步到当前播放列表");
									favouritePlayList.addToPlayList(song);
								}
							}
							//显示播放列表中的信息
					...
					case 3:
						...
						PlayList pl = plc.searchPlayList(strPlayListName1);
						if(pl==null) {
						...
						break;
					case 4:
						System.out.println("通过歌曲名称查询播放列表中的歌曲");break;
					case 5:
						System.out.println("修改播放列表中的歌曲 ");break;
					case 6:
						System.out.println("删除播放列表中的歌曲");break;
					case 7:
						System.out.println("显示播放列表中的歌曲");break;
					default:
						System.out.println("该数字没有对应的操作");break;
					}
				}
				break;
			case 2:
				...
				while(true) {
					...
					case 1:
						...
						//创建一个新的播放列表对象
						favouritePlayList = new PlayList(playName);
						//将播放列表添加到播放器
						plc.addPlayList(favouritePlayList);
						break;
					case 2:
						...
						//查询播放列表是否存在
						PlayList playList1 = plc.searchPlayList(strPlayListName);
						if(playList1==null) {
							System.out.println("该播放列表不存在");
						}else {
					...
					case 3:
						...
						PlayList playList2 = plc.searchPlayList(strPlayList1);
						if(playList2==null) {
							System.out.println("该播放列表不存在");
						}else {
